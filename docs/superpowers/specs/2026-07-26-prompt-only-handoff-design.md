...
"You are taking over this session from _<Source>_." and carries the instructions the
...
If the assembled prompt would exceed 30,000 characters in mode 2, the _range list_ is
...
delivers a large multi-line prompt as a bracketed paste and _does not submit it_. Measured
...
_Not verifiable on this machine._ opencode 1.18.5 crashes on startup here, dumping a Bun
...
worked and saying otherwise would be a lie. But opencode leaves its _pane_ behind with no
...
claude's `❯` and grok's `>` sit _inside_ border lines, so they are not at line start and a
...
- `NOT_READY` is matched only against the text from that line to the end of the capture,
  normalised. Phrases above it are history; a notice in a footer _below_ it still counts.
