...
function isLiveShim(file) {
  // Resolve first: a PATH entry may be a symlink to a text shim elsewhere,
  // whose node_modules reference is relative to the real file location, not
  // the symlink. Resolving keeps that check against the right directory, so
  // a live symlinked shim is not misreported as dangling (or vice versa).
  // A failure here means dangling link (or race), which is not installed —
  // including a dangling link that itself ends in .exe/.com.
  let resolved;
  try {
  ...
  if (/[.](exe|com)$/i.test(file) || /[.](exe|com)$/i.test(resolved))
    return true;
  let text;
  try {
  ...
  if (!ref) return true;
  const target = path.resolve(path.dirname(resolved), ref[0]);
