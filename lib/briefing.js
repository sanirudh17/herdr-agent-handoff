...
2. Send a status message — **one or two lines**: what the previous agent completed, the current stopping point, and that you are ready for the user's next instruction. Send it before anything else.
3. STOP. Do not search, read workspace files, edit, run commands, update todos, or touch the todo list, and do not re-verify by re-implementing. Wait for the user's next instruction; this handoff is a context transfer, not an order to begin. (The transcript file itself is step 1.)
...
3. **No tools until the user directs.** No searching, reading, editing, commands, or todo updates — before the status message or after it — until the user replies with the next instruction.
