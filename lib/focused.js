...
function noteEntry(obj, found, fallback, rolesByMessage) {
  ...
  const text = (unwrapped ? entryText(unwrapped.entry) : entryText(obj)).trim();
  if (!text) return;
  if (isUserRole(role)) {
...
function finishExcerpts(found, fallback) {
  if (!found.firstUser && (fallback.summary || fallback.title)) {
    found.firstUser = String(fallback.summary || fallback.title);
  }
  return found;
}
