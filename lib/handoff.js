...
// Marker matching always goes through flattened text on both sides, so a marker
// broken across a wrapped line still counts.
const showsMarker = (screen, markers) =>
  markers.some((marker) => flat(screen).includes(flat(marker)));
...
// after it stops being true. When a prompt marker (like > or ? for shortcuts) is
// present, only phrases appearing *after* the prompt marker indicate active
// startup state; static banner text above the prompt line is past scrollback.
...
function textAfterLastPrompt(screen) {
  ...
  // Check if screen begins directly with a prompt like "> "
  if (lastIdx === -1) {
    if (screen.startsWith("> ") || screen.startsWith(">")) {
      lastIdx = 0;
      markerLen = screen.startsWith("> ") ? 2 : 1;
    }
  }
  return lastIdx !== -1 ? screen.slice(lastIdx + markerLen) : null;
}

function startingUp(screen) {
  if (!screen) return false;
  const afterPrompt = textAfterLastPrompt(screen);
  if (afterPrompt !== null) {
    return matches(flat(afterPrompt), NOT_READY);
  }
  return matches(flat(tailOf(screen)), NOT_READY);
}
