...
function timings(env) {
  ...
  return {
    ...
    still: num("HANDOFF_STILL_MS", 3000),
...
// Readiness is judged from the target's screen going still, not from its reported
// state. Herdr calls Antigravity "idle" from the moment it appears, while it is in
// fact signing in and discarding everything typed at it; its screen, meanwhile,
// keeps changing until it is genuinely at a prompt. A settled screen is the
// generic signal, and it costs nothing for agents that start instantly.
async function waitUntilScreenSettles(call, paneId, stillMs, capMs) {
  ...
  for (;;) {
    ...
    if (now !== null && now !== last) {
      last = now;
      since = Date.now();
    } else if (now !== null && Date.now() - since >= stillMs) {
      log(`target screen settled after ${Date.now() - (deadline - capMs)}ms`);
      return;
    }
    if (Date.now() >= deadline) {
      log("target screen never settled; sending anyway");
      return;
    }
  }
}
...
async function deliverPrompt(call, paneId, text, env, markers) {
  ...
  await waitUntilScreenSettles(call, paneId, t.still, t.readyCap);
