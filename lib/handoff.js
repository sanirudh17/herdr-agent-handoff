...
function reacted(before, now) {
  ...
  return now.seq > before.seq || now.status !== before.status;
}

// Three signals, any one of which proves the prompt landed. No single one covers
// every agent:
//
//   - the prompt echoed on screen. Exact, and useless for compact TUIs: measured
//     against Antigravity, a 222-character kickoff never becomes visible in any
//     snapshot source, though a short one does. opencode is the same shape.
//   - the screen changing from the moment before submission. Works for anything
//     that draws a response at all — Antigravity's screen changes 365ms after the
//     prompt it will not echo.
//   - the agent's reported state changing.
//
// The last two would be unsafe on their own if the target were still starting up,
// since that churns by itself. They are trustworthy here only because nothing is
// submitted until the screen has held still and the agent reports idle: a change
// measured from that baseline is a change we caused.
//
// This is also what lets an agent nobody has tested work: none of it knows
// anything about a particular agent.
async function confirmDelivery(call, paneId, markers, windowMs, baseline) {
  const deadline = Date.now() + windowMs;
  for (;;) {
    const screen = readScreen(call, paneId);
    if (screen !== null) {
      if (markers.some((marker) => screen.includes(normalize(marker)))) {
        return "the prompt is on the target's screen";
      }
      if (baseline.screen !== null && screen !== baseline.screen) {
        return "the target redrew in response";
      }
    }
    if (reacted(baseline.agent, agentSnapshot(call, paneId))) {
      return "the target changed state in response";
    }
...
const startingUp = (screen) => matches(screen, NOT_READY);
const needsAnswer = (screen) => matches(screen, NEEDS_ANSWER);
...
async function waitUntilReady(call, paneId, opts) {
  ...
  for (;;) {
    ...
    if (!blocked && !busy && status !== "unknown" &&
        (stable || Date.now() - started >= graceMs)) {
...
async function deliverPrompt(call, paneId, text, env, markers, hooks = {}) {
  ...
  // Captured immediately before submission, so any later difference is ours.
  const baseline = {
    screen: readScreen(call, paneId),
    agent: agentSnapshot(call, paneId),
  };
  ...
  const proof = await confirmDelivery(call, paneId, markers, t.confirmWindow, baseline);
  if (proof) {
    log(`prompt delivered — ${proof}`);
    return;
  }
...
module.exports = {
  run, MESSAGES, shellIsAtPrompt, startingUp, needsAnswer, NOT_READY, NEEDS_ANSWER,
