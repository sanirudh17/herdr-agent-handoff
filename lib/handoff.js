...
async function deliverPrompt(call, paneId, text) {
  const attempt = () =>
    call(["agent", "prompt", paneId, text, "--wait", "--until", "working",
      "--timeout", String(DELIVERY_TIMEOUT_MS)]);

  await sleep(SETTLE_MS);
  try {
    attempt();
    log("prompt delivered; target is working");
    return;
  } catch (err) {
    // `timeout` means the agent did change state but had not reached "working"
    // within the window: the text landed, the turn is just slower than the wait.
    if (err && err.code === "timeout") {
      log('delivery wait ended without "working"; the prompt landed, treating as delivered');
      return;
    }
    // Anything other than a stall is a real failure, not a slow turn.
    if (!err || err.code !== "agent_prompt_stalled") throw err;
    log("submission stalled — nothing reached the agent; retrying once");
  }

  await sleep(RETRY_SETTLE_MS);
  attempt();
  log("prompt delivered on retry");
}
...
async function run(opts) {
  ...
  try {
    await deliverPrompt(call, targetPaneId, briefing.kickoff({ sourceName, handoffPath }));
  } catch (err) {
