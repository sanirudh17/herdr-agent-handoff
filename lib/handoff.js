...
const focused = require("./focused.js");
...
const HANDOFF_MODES = ["focused", "full"];

const MESSAGES = {
  notAgentPane: "Handoff unavailable: the active pane is not a running agent.",
  focusedFailed: (reason) =>
    reason
      ? `Focused handoff unavailable: could not build a focused summary. (${reason})`
      : "Focused handoff unavailable: could not build a focused summary.",
  noContext: (reason) =>
    reason
      ? `Full handoff unavailable: complete session context could not be retrieved for this source agent. (${reason})`
  ...
  startingUp: (name) =>
    `${name} is still starting up — the handoff will be delivered when it is ready.`,
  needsAttention: (name) =>
...
function resolveTheme(env, call) {
  ...
  return theme.resolveTheme(
    keybindings.findConfigPath({ env, helpOutput: help }),
  );
}
...
function uniqueAgentName(call, kind) {
  ...
  try {
    ...
    // agent list unavailable; start from the base name
  }
...
function timings(env) {
  ...
  return {
    ...
    // Antigravity paints its TUI before it begins its delayed Google AI
    // subscription/credit check. Its input box looks usable during that check but
    // swallows a paste, so hold the prompt for a full quiet window measured from
    // the first rendered TUI frame — never from process detection or pane launch.
    agyTuiSettle: num("HANDOFF_AGY_TUI_SETTLE_MS", 15000),
    ...
    // Some agents persist their transcript lazily (pi writes its session file
    // only after the first assistant reply) or re-index on a delay (grok). A
    // missing file at the moment of handoff is therefore worth waiting for.
    resolveRetries: num("HANDOFF_RESOLVE_RETRIES", 12),
    resolveRetryMs: num("HANDOFF_RESOLVE_RETRY_MS", 750),
  };
}
...
function launchCommand(definition, kind) {
  const name = (definition && definition.execName) || kind;
  const args = (definition && definition.yoloArgs) || [];
  const command = [name, ...args].join(" ");
  // Cline's global npm installation can start its self-updater alongside a new
  // TUI. Old builds replace the live Hub while doing that, leaving the TUI unable
  // to receive a handoff. Disable only this automatic check for handoff-launched
  // Cline sessions; manual `cline update` remains available to the user.
  if (kind === "cline") {
    return process.platform === "win32"
      ? `$env:CLINE_NO_AUTO_UPDATE='1'; ${command}`
      : `CLINE_NO_AUTO_UPDATE=1 ${command}`;
  }
  if (kind === "agy" && process.platform === "win32") {
    return `$env:TERM=''; ${command}`;
  }
  return command;
}

function hasLaunchArgs(definition) {
  return Boolean(
    definition && definition.yoloArgs && definition.yoloArgs.length,
  );
}
...
async function waitForShellPrompt(call, paneId, timeoutMs) {
  ...
  for (;;) {
    try {
      if (
        shellIsAtPrompt(
          call(["pane", "process-info", "--pane", paneId]).process_info,
        )
      ) {
        return true;
      }
...
async function waitForDetection(call, paneId, timeoutMs) {
  ...
  for (;;) {
    try {
      ...
      if (pane.agent) {
        log(
          `detected ${pane.agent} in ${paneId} (status ${pane.agent_status})`,
        );
        return true;
      }
...
async function startAgent(
  call,
  { name, kind, paneId, command, definition, env },
) {
  // Herdr's `agent start` API starts a kind with its registered default command;
  // it cannot forward the agent-specific bypass arguments. Keep it for the
  // agents that need no switch, but use the pane shell everywhere a YOLO command
  // must be run (including POSIX).
  if (!windowsAgentStartIsBroken(env) && !hasLaunchArgs(definition)) {
...
const isMissing = (err) =>
  Boolean(err) &&
  (err.code === "agent_not_found" || err.code === "pane_not_found");
...
const showsMarker = (screen, markers) => {
  ...
  return markers.some(
    (marker) =>
      flattened.includes(flat(marker)) || squashed.includes(squash(marker)),
  );
};
...
async function busyFor(call, paneId, ms, statusBefore) {
  ...
  if (usable(screen) && (startingUp(screen) || needsAnswer(screen)))
    return false;
  // A single reading after the delay could catch a momentary "working" blip
  // during agent churn. Confirm with a second read a short interval later so
  // only sustained busyness counts as proof of delivery.
  if (!busy()) return false;
  await sleep(400);
  const screen2 = readScreen(call, paneId);
  if (usable(screen2) && (startingUp(screen2) || needsAnswer(screen2)))
    return false;
  return busy();
}
...
const alternateScreenReadUnavailable = (err) =>
  /alternate-screen history can only be captured by scrolling while idle|use --source visible/i.test(
    describeError(err),
  );

function readScreen(call, paneId) {
  const read = (source) =>
    call(["agent", "read", paneId, "--source", source, "--lines", "400"], {
      json: false,
    });
  try {
    return read("recent-unwrapped");
  } catch (err) {
    if (isMissing(err)) throw new TargetGone("target closed");
    // Cline works in an alternate screen. Herdr correctly refuses to scroll that
    // history while Cline is working, but explicitly offers the current visible
    // frame instead. Treating the refusal as an empty screen made the readiness
    // loop wait its entire cap even though the TUI was already available.
    if (alternateScreenReadUnavailable(err)) {
      try {
        return read("visible");
      } catch (visibleErr) {
        if (isMissing(visibleErr)) throw new TargetGone("target closed");
        log(`visible screen read failed: ${describeError(visibleErr)}`);
        return null;
      }
    }
...
async function waitUntilReady(call, paneId, opts) {
  ...
  let since = Date.now();
  // Initialize to Infinity so the grace period starts from when this function
  // is called, not from when the TUI first painted (which may have been during
  // the settle window). Without this, a screen that was already painted when
  // waitUntilReady starts would have its grace period already elapsed, allowing
  // the function to return before the agent has truly settled.
  let firstPainted = Infinity;
  ...
  for (;;) {
    ...
    const lines = typeof last === "string" ? last.split("\n") : [];
    const hasPrompt = painted && inputLineIndex(lines) !== -1;
    const blocked = status === "blocked" || needsAnswer(last);
    const busy = startingUp(last) || (status === "working" && !hasPrompt);
...
async function waitForAgyTui(call, paneId, t, onSlow) {
  const started = Date.now();
  let warnedSlow = false;
  for (;;) {
    const screen = readScreen(call, paneId);
    if (usable(screen)) {
      log(
        `Antigravity TUI painted after ${Date.now() - started}ms; holding handoff text for ${t.agyTuiSettle}ms`,
      );
      await sleep(t.agyTuiSettle);
      return;
    }
    const elapsed = Date.now() - started;
    if (!warnedSlow && elapsed >= SLOW_NOTICE_MS) {
      warnedSlow = true;
      if (onSlow) onSlow();
    }
    if (elapsed >= t.readyCap) {
      throw new TargetNotReady(
        "Antigravity did not render its TUI, so the handoff prompt was never pasted",
      );
    }
    await sleep(400);
  }
...
async function deliverPrompt(call, paneId, text, env, markers, hooks = {}) {
  ...
  if (hooks.kind === "agy") await waitForAgyTui(call, paneId, t, hooks.onSlow);
  ...
  for (let attempt = 1; attempt <= t.attempts; attempt += 1) {
    const outcome = await submitOnce(
      call,
      paneId,
      text,
      env,
    ...
    log(
      `the target discarded the prompt while starting up; waiting for it to finish (attempt ${attempt})`,
    );
  }
}
...
async function submitOnce(call, paneId, text, env, markers, hooks, t) {
  ...
  if (usable(finalLook) && needsAnswer(finalLook)) {
    throw new TargetNeedsAttention(
      "the target put a question up before the prompt was sent",
    );
  }
  ...
  try {
    ...
    if (isMissing(err))
      throw new TargetGone("target closed before the prompt was sent");
    throw err;
  }

  // Fast-path rejection: Antigravity can start a late "Verifying your account…"
  // redraw in the narrow gap between the final look and the prompt submission.
  // The prompt lands during that window, is briefly echoed, then swallowed. A
  // short pause and a single screen read catch this without waiting for the full
  // confirmation flow — if the screen already shows a startup phrase, the prompt
  // did not survive and there is no point waiting for it to appear.
  await sleep(500);
  const postSubmit = readScreen(call, paneId);
  if (usable(postSubmit) && startingUp(postSubmit)) {
    log(
      "prompt submitted during a late startup redraw; discarding immediately",
    );
  ...
  if (!(await confirmDelivery(call, paneId, markers, t.nudge))) {
    ...
    if (usable(parked) && !needsAnswer(parked)) {
      try {
        ...
        log(
          "the prompt was left unsent in the composer; submitted it with one Enter",
        );
      } catch (err) {
        if (isMissing(err))
          throw new TargetGone("target closed before the prompt was submitted");
  ...
  if (await confirmDelivery(call, paneId, markers, t.confirmWindow)) {
    ...
    if (
      !stillThere &&
      usable(after) &&
      (startingUp(after) || needsAnswer(after))
    ) {
      return { discarded: true };
    }
  ...
  if (usable(why)) {
    ...
    if (needsAnswer(why)) {
      throw new TargetNeedsAttention(
        "the target is waiting on you, not on the handoff",
      );
    }
...
// A session whose record is missing right now can appear moments later (pi and
// cline flush their transcript on the first exchange; grok indexes on its own
// schedule). Only missing-record failures are retried, bounded, and only the
// prompt-building step is affected — nothing is created until this settles.
const TRANSIENT_RESOLVE =
  /does not exist|no session file|is empty|has no lines|no transcript file|no session file for|no persisted session|not indexed|not been indexed/i;

async function resolveSourceWithRetry(attempt, env, t) {
  for (let i = 0; ; i += 1) {
    try {
      return attempt();
    } catch (err) {
      const reason = describeError(err);
      if (i >= t.resolveRetries || !TRANSIENT_RESOLVE.test(reason)) throw err;
      log(
        `source not resolvable yet (${reason}); waiting for the agent to persist its transcript (attempt ${i + 1}/${t.resolveRetries})`,
      );
      await sleep(t.resolveRetryMs);
    }
  }
}

function isHandoffMode(value) {
  return value === "focused" || value === "full";
}

function openPane(call, env, { entrypoint, requestFile, placement }) {
  const openArgs = (place) => [
    "plugin",
    "pane",
    "open",
    "--plugin",
    ...
    "--cwd",
    paths.pluginRoot(env),
    "--env",
    `HERDR_HANDOFF_REQUEST=${requestFile}`,
    "--focus",
  ];
  try {
    call(openArgs(placement ?? null));
    return true;
  } catch {
...
async function openModePicker({
  ...
  ipc.writeJson(requestFile, {
    resultPath: resultFile,
    contextLine,
    theme: themeValue,
  });
  const opened = openPane(call, env, {
    entrypoint: "mode-picker",
    requestFile,
  });
  if (!opened) {
  ...
  try {
    return await ipc.waitForResult(resultFile, { timeoutMs: pickerTimeoutMs });
  } finally {
    ipc.cleanup([requestFile, resultFile]);
  }
}

async function resolveFocusedSummaryText({
  opts,
  env,
  call,
  sourcePaneId,
  ...
  if (typeof opts.focusedSummary === "string" && opts.focusedSummary.trim()) {
    return opts.focusedSummary.trim();
  }
  if (
    typeof env.HANDOFF_FAKE_FOCUSED_SUMMARY === "string" &&
    env.HANDOFF_FAKE_FOCUSED_SUMMARY.trim()
  ) {
    return env.HANDOFF_FAKE_FOCUSED_SUMMARY.trim();
  }
  const generator =
    opts.generateFocusedSummary ||
    opts.focusedSummaryGenerator ||
    opts.createFocusedSummary;
  if (typeof generator === "function") {
    const out = await generator({ call, sourcePaneId, meta, env, session });
    if (typeof out === "string") return out.trim();
    if (out && typeof out.summary === "string") return out.summary.trim();
    throw new Error("the focused summary generator returned nothing usable");
  }
  // Default: derive the summary from the resolved transcript. The live source
  // pane is never prompted, so the handoff cannot land back in it.
  return focused.summarizeSession({ meta, session });
}

// A target that resolves to the source pane is never typed into. Delivery to
// the source would look exactly like "the handoff ran in the same agent", so
// this fails loudly instead of delivering to the wrong place.
function isSamePane(a, b) {
...
async function run(opts) {
  ...
  const paneCwd = pane.cwd || ctx.focused_pane_cwd || homedir;

  // The native reference plus, when that fails, the pane's working directory:
  // the recovery resolvers in sources.js key off it (cline and grok have no
  // native reference at all; pi may not have flushed its file yet).
  const resolveSource = () =>
    sources.resolve({
      agent: sourceKind,
  ...
  // 2. Try to resolve complete context BEFORE the picker opens, so the header
  //    can name the session size. A failure here no longer stops the handoff:
  //    the picker opens either way, the source is re-resolved after the choice
  //    (with a wait for agents that persist lazily), and only then — still
  //    before anything is created — is the handoff refused.
  let resolved = null;
  let contextProblem = null;
  try {
    resolved = resolveSource();
  } catch (err) {
    contextProblem = describeError(err);
    log(
      `context unresolved before the picker (${contextProblem}); opening the picker anyway`,
    );
  }
  ...
  const notInstalled = agents.AGENTS.filter(
    (a) => !installedKinds.has(a.kind),
  ...
  const requestFile = ipc.requestPath(requestsDir, id);
  const resultFile = ipc.resultPath(requestsDir, id);
  const themeValue = resolveTheme(env, call);
  const request = {
    resultPath: resultFile,
    contextLine,
    destination: destinationText,
    installed: installedAgents.map(({ kind, name, isSource }) => ({
      kind,
      name,
      isSource,
    })),
    notInstalled,
    // Present when the source session could not be resolved yet; the picker
    // still opens, and the handoff is re-resolved after the choice.
    contextUnavailable: contextProblem,
  ...
  const meta = {
    ...
    destination,
    strategy: resolved ? resolved.strategy : null,
  };

  // 4. Choose the handoff context mode, then the target. Focused is the
  //    default/highlighted option in the mode picker; full preserves the
  //    historical behavior exactly.
  let selectedMode = null;
  if (isHandoffMode(handoffMode)) {
    selectedMode = handoffMode;
  } else if (
    isHandoffMode(env.HANDOFF_HANDOFF_MODE) ||
    isHandoffMode(env.HANDOFF_FAKE_HANDOFF_MODE)
  ) {
    selectedMode = isHandoffMode(env.HANDOFF_HANDOFF_MODE)
      ? env.HANDOFF_HANDOFF_MODE
      : env.HANDOFF_FAKE_HANDOFF_MODE;
  } else if (modeChoice) {
    if (modeChoice.cancelled) {
      return { ok: false, cancelled: true, message: "" };
    }
    if (isHandoffMode(modeChoice.mode)) selectedMode = modeChoice.mode;
  }
  if (!selectedMode) {
    if (dryRun) {
      selectedMode = "full";
    } else if (pickerChoice) {
      // Backward compatibility: historical tests drive the target picker
      // directly without a mode seam. They exercise full-transcript behavior.
      selectedMode = "full";
    } else {
      const modeResult = await openModePicker({
        call,
        env,
        contextLine,
        themeValue,
      ...
      if (
        !modeResult ||
        modeResult.cancelled ||
        !isHandoffMode(modeResult.mode)
      ) {
        return { ok: false, cancelled: true, message: "" };
      }
  ...
  // 6. Build the prompt for the selected mode.
  //
  //    full: the historical behavior exactly — measure the session,
  //    re-resolving so the capture is as fresh as possible. Nothing is
  //    written: the transcript either travels inside the prompt or is read by
  //    the target from where its own agent put it.
  //
  //    focused: resolve and measure the same way, then derive a concise
  //    summary from the transcript without ever prompting the live source
  //    pane. The summary travels as the main prompt; the transcript
  //    reference, when one safely exists, travels as an explicitly optional
  //    fallback only.
  ...
  if (selectedMode === "focused") {
    try {
      resolved = await resolveSourceWithRetry(resolveSource, env, timings(env));
      ...
      const message =
        err instanceof SqliteUnavailable && /node:sqlite/.test(err.message)
          ...
          : MESSAGES.noContext(reason);
      notify(call, message);
      return { ok: false, message, detail: reason };
    }
    try {
      focusedSummary = await resolveFocusedSummaryText({
        opts,
        env,
        call,
        sourcePaneId,
      ...
      const reason = describeError(err);
      const message = MESSAGES.focusedFailed(reason);
      notify(call, message);
      return { ok: false, message, detail: reason };
    }
    if (!focusedSummary || !String(focusedSummary).trim()) {
      const reason = "no focused summary could be produced";
      const message = MESSAGES.focusedFailed(reason);
      notify(call, message);
      return { ok: false, message, detail: reason };
    }
    focusedSummary = String(focusedSummary).trim();
    built = briefing.buildFocused({ meta, summary: focusedSummary, session });
    if (!built) {
      const reason = "the focused summary does not fit the prompt budget";
      const message = MESSAGES.focusedFailed(reason);
      notify(call, message);
      return { ok: false, message, detail: reason };
    }
    ...
    built = briefing.buildFull({ meta, session });
    if (!built) {
      const reason = !session.nativePath
        ? "the session store has no file to point the target at"
        : !session.readable
          ? "the session is too large to inline and not readable as text"
          : "the session is too large to reference safely";
      const message = MESSAGES.noContext(reason);
      notify(call, message);
      return { ok: false, message, detail: reason };
    }
  ...
  // The handoff must land in the new pane, never back in the source. If pane
  // creation ever resolves to the source id, stop loudly instead of typing the
  // handoff into the session it came from.
  if (isSamePane(targetPaneId, sourcePaneId)) {
    log(
      `refusing to hand off: target pane is the source pane (${sourcePaneId})`,
    );
    const message = MESSAGES.targetCreateFailed(destLabel(destination));
    notify(call, message);
    return {
      ok: false,
      message,
      detail: "target pane resolved to the source pane",
    };
  }

  // 8. Start the target agent.
  ...
  try {
    await startAgent(call, {
      ...
      kind: targetKind,
      paneId: targetPaneId,
      definition: targetDefinition || targetDef,
      command: launchCommand(targetDefinition || targetDef, targetKind),
      env,
    });
  } catch (err) {
  ...
  // 9. Deliver the handoff. Addressed by pane id, which always identifies the
  //    agent; the name is only a sidebar label and may not have been applied.
  //    The target was verified above to differ from the source; re-check here
  //    so no future edit can silently route the prompt back into the source.
  if (isSamePane(targetPaneId, sourcePaneId)) {
    log(`refusing delivery: target pane is the source pane (${sourcePaneId})`);
  ...
  try {
    ...
    if (err instanceof TargetGone || isMissing(err)) {
      ...
      try {
        ...
        // pane get failed; treat as "the pane is gone"
      }
      if (paneSurvives) {
...
module.exports = {
  ...
  launchCommandForTest: launchCommand,
