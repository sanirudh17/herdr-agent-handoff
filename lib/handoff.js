...
const MESSAGES = {
  ...
  focusedFailed: (reason) =>
    reason
      ? `Focused handoff unavailable: could not generate a focused summary from the source agent. (${reason})`
      : "Focused handoff unavailable: could not generate a focused summary from the source agent.",
  noContext: (reason) =>
    reason
...
async function resolveFocusedSummaryText({
  ...
  if (typeof generator === "function") {
    const out = await generator({ call, sourcePaneId, meta, env });
  ...
  return focused.generateFocusedSummary({ call, sourcePaneId, meta, env });
}
...
async function run(opts) {
  ...
  //
  //    focused: ask the source agent for a concise summary, then build a
  //    focused prompt that never embeds the transcript body. The transcript
  ...
  if (selectedMode === "focused") {
    ...
    if (!focusedSummary || !String(focusedSummary).trim()) {
      const reason = "the source agent produced an empty summary";
      const message = MESSAGES.focusedFailed(reason);
      notify(call, message);
      return { ok: false, message, detail: reason };
    }
    ...
    try {
      ...
      log(
        `focused fallback context unavailable (${describeError(err)}); continuing without a transcript reference`,
      );
      session = null;
    }
