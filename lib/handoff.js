...
function timings(env) {
  ...
  return {
    ...
    // How long the agent must hold one state before it counts as waiting for input.
    quiet: num("HANDOFF_QUIET_MS", 1500),
    readyCap: num("HANDOFF_READY_CAP_MS", 45000),
    // Per attempt. The window doubles as the spacing between attempts, and the
    // toast is withheld until the prompt is actually visible — announcing
    // "handoff started" over an agent that has not read it is the bug this
    // replaces. Six 20s attempts cover a two-minute first-run sign-in.
    confirmWindow: num("HANDOFF_CONFIRM_WINDOW_MS", 20000),
    attempts: num("HANDOFF_DELIVERY_ATTEMPTS", 6),
  };
}
...
// A target still signing in churns through states; once it is genuinely waiting
// for input it goes quiet. Waiting for that quiet is what stops the prompt being
// typed into a screen that is not listening yet.
async function waitUntilQuiet(call, paneId, quietMs, capMs) {
  const deadline = Date.now() + capMs;
  let last = agentSnapshot(call, paneId);
  let since = Date.now();
  for (;;) {
    await sleep(300);
    ...
    if (!last || now.seq !== last.seq || now.status !== last.status) {
      last = now;
      since = Date.now();
...
async function deliverPrompt(call, paneId, text, env, markers) {
  const t = timings(env);
  await sleep(t.settle);
  await waitUntilQuiet(call, paneId, t.quiet, t.readyCap);

  // Antigravity discards anything typed before it has finished signing in —
  // measured: a lone submission five seconds in never appeared, while a later one
  // did. So submit, wait for it to show, and try again if it does not. Each
  // attempt is confirmed before the next, so the loop stops the moment the prompt
  // lands and cannot pile up copies behind a confirmation it already had.
  let lastError = null;
  for (let attempt = 1; attempt <= t.attempts; attempt += 1) {
    try {
      ...
      lastError = err;
      log(`submission failed on attempt ${attempt}: ${describeError(err)}`);
    }

    if (await confirmDelivery(call, paneId, markers, t.confirmWindow)) {
      log(`prompt confirmed on the target's screen (attempt ${attempt})`);
      return;
    }

    log(
      `attempt ${attempt}: the prompt has not appeared` +
      (attempt < t.attempts ? " — the target was not listening yet, trying again" : "")
    );
  }

  throw lastError || new Error("the target never showed the handoff prompt");
}
...
async function run(opts) {
  ...
  try {
    ...
    await deliverPrompt(call, targetPaneId, kickoff, env, [
      "HANDOFF.md",
