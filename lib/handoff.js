...
function hasLaunchArgs(definition) {
  return Boolean(definition && definition.yoloArgs && definition.yoloArgs.length);
}
...
async function startAgent(call, { name, kind, paneId, command, definition, env }) {
...
async function busyFor(call, paneId, ms, statusBefore) {
  ...
  if (usable(screen2) && (startingUp(screen2) || needsAnswer(screen2))) return false;
  return busy();
}
...
async function submitOnce(call, paneId, text, env, markers, hooks, t) {
  ...
  if (usable(postSubmit) && startingUp(postSubmit)) {
    log("prompt submitted during a late startup redraw; discarding immediately");
...
const TRANSIENT_RESOLVE = /does not exist|no session file|is empty|has no lines|no transcript file|no session file for|no persisted session|not indexed|not been indexed/i;
...
async function run(opts) {
  ...
  const resolveSource = () =>
    sources.resolve({ agent: sourceKind, sessionRef: pane.agent_session, cwd: paneCwd, env, homedir });
  ...
  try {
    ...
    log(`context unresolved before the picker (${contextProblem}); opening the picker anyway`);
  }
  ...
  const size = resolved && resolved.lines
...
module.exports = {
  ...
  inputLineIndex, NOT_READY, NEEDS_ANSWER, timings,
