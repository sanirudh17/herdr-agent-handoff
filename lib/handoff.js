...
function timings(env) {
  ...
  return {
    ...
    // Antigravity exposes an input box before its account/credit-tier check is
    // actually complete. A paste during that window flashes briefly, is dropped,
    // and then the pane reports that verification is still in progress. Its normal
    // command prompt accepts input after roughly seven seconds, so hold handoff
    // text back for a conservative 10 seconds.
    agySettle: num("HANDOFF_AGY_SETTLE_MS", 10000),
...
async function deliverPrompt(call, paneId, text, env, markers, hooks = {}) {
  const t = timings(env);
  const settleMs = hooks.kind === "agy" ? t.agySettle : t.settle;
