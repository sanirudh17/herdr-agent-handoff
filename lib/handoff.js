...
// The accent Herdr paints its own popup border with, so the modal matches the
// user's theme instead of a flat white highlight.
function resolveAccent(env, call) {
  let help = "";
  try {
  ...
  return theme.resolveAccent(keybindings.findConfigPath({ env, helpOutput: help }));
}
