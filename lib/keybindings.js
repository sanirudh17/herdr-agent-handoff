...
function patch(text, opts = {}) {
  ...
  const original = text;
  const active = activeLines(text);
  const alreadyInstalled =
    ACTIONS.every((action) => active.some((line) => line.includes(`"${action}"`))) &&
    KEYS.every((key) => active.some((line) => line.includes(`"${key}"`)));
  ...
  return { text: next, changed: true, conflicts: [] };
}
