...
const STORES = {
  ...
  agy: {
    ...
    fileMatch: (base, id) =>
      base.includes(id) && (base.endsWith(".jsonl") || base.endsWith(".json")),
  },
  ...
  cline: {
    ...
    verified: false,
    roots: (home, env) => [
      join(env.CLINE_DATA_DIR || join(home, ".cline", "data"), "sessions"),
    ],
  ...
  grok: {
    ...
    verified: false,
    roots: (home, env) => [
      join(env.GROK_DATA_DIR || join(home, ".grok"), "sessions"),
    ],
    recover: recoverGrok,
...
function recoverOpencode(homedir, cwd, env) {
  ...
  if (
    matches.length > 1 &&
    matches[1].time_updated === matches[0].time_updated
  ) {
    throw new SourceContextUnavailable(
...
function recoverCline(homedir, cwd, env) {
  ...
  if (!rows) {
    const sessionsRoot = join(
      env.CLINE_DATA_DIR || join(homedir, ".cline", "data"),
      "sessions",
    );
    let entries;
    try {
    ...
    for (const entry of entries) {
      ...
      try {
        meta = JSON.parse(
          fs.readFileSync(join(sessionsRoot, id, `${id}.json`), "utf8"),
        );
      } catch {
        continue;
      }
  ...
  if (
    candidates.length > 1 &&
    candidates[1].started === candidates[0].started
  ) {
    throw new SourceContextUnavailable(
  ...
  if (!fs.existsSync(messages)) {
    throw new SourceContextUnavailable(
      `cline session ${best.id} has no transcript file yet`,
    );
  }
...
function recoverGrok(homedir, cwd, env) {
  ...
  if (!hasSqlite()) {
    throw new SourceContextUnavailable(
      "reading grok's session store requires Node 22.5 or newer",
    );
  }
  const { DatabaseSync } = require("node:sqlite");
  let best;
  try {
    ...
    try {
      const stored = db
        .prepare(
          "SELECT cwd, MAX(updated_at) AS newest FROM session_docs GROUP BY cwd",
        )
        .all()
      ...
      const rows = db
        .prepare(
          "SELECT session_id, updated_at, content FROM session_docs WHERE cwd = ? ORDER BY updated_at DESC",
        )
        .all(stored[0].cwd);
...
function resolveAgyFallback(homedir, env, id) {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const target = fs.existsSync(sysLog)
        ? sysLog
        : fs.existsSync(directLog)
          ? directLog
          : null;
      if (target) {
        try {
          ...
          if (stat.size > 0)
            candidates.push({ file: target, mtime: stat.mtimeMs });
        } catch {
          // ignore
        }
      }
...
function resolve({
  ...
  const primary = () => {
    if (store.strategy === "sqlite") {
      ...
      if (!dbPath) {
        throw new SourceContextUnavailable(
          `${agent} session database not found`,
        );
      }
