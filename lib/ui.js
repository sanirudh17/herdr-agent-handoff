...
const { accentCodes } = require("./theme.js");
...
// The accent is the theme's own colour, so selected chrome matches Herdr's popup
// border instead of the flat white a reverse-video bar produced.
function accentOf(state) {
  const codes = accentCodes(state.accent);
  // Dark text on the accent fill, the way Herdr's own "apply" chip renders it.
  // Theme accents are light more often than not, so inheriting the default
  // foreground would leave light-on-light.
  return { bar: codes.bg + BLACK + BOLD, text: codes.fg + BOLD };
}
...
function headerLine(state, width, styled, total) {
  ...
  const text = " " + variant.labels
    .map((l) => style(l.text, l.active ? accent.bar : DIM, styled))
    .join(" ") + " ";
...
function footerLine(state, width, styled) {
  ...
  const accent = accentOf(state);
  const render = (list) =>
    list.map((c) => style(c.label, c.primary ? accent.bar : DIM, styled)).join(" ");
...
function renderFrame(state, opts = {}) {
  ...
  for (const [offset, agent] of visible.entries()) {
    ...
    if (isCursor) {
      lines.push(style(pad(row, width), accentOf(state).bar, styled));
    } else if (selectable) {
      lines.push(row);
    } else {
...
function renderChosenFrame(state, opts = {}) {
  ...
  frame[frame.length - 1] = style(truncate(message, state.width), accentOf(state).text, styled);
  return frame;
}
