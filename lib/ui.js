...
// The available list keeps its full budget whether or not the unavailable block
// is showing. Revealing agents you cannot pick must never push away agents you
// can — the unavailable block takes only the rows left over.
function viewportSize(state) {
  return Math.max(1, state.height - HEADER_ROWS - FOOTER_ROWS);
}

function unavailableBudget(state) {
  if (!state.showUnavailable) return 0;
  const shown = Math.min(state.available.length, viewportSize(state));
  const spare = state.height - HEADER_ROWS - FOOTER_ROWS - shown;
  // One row goes to the "not installed:" caption.
  return Math.max(0, Math.min(spare - 1, MAX_UNAVAILABLE_SHOWN));
}
...
function renderFrame(state) {
  ...
  if (state.showUnavailable) {
    const budget = unavailableBudget(state);
    const shown = state.unavailable.slice(0, budget);
    const rest = state.unavailable.length - shown.length;
    lines.push(rest > 0 ? `  not installed (${rest} more):` : "  not installed:");
