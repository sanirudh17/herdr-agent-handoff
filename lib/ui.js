...
const SGR_MOUSE = /^\x1b\[<(\d+);(\d+);(\d+)([Mm])/; // eslint-disable-line no-control-regex -- terminal escape parsing
...
function decodeInput(buffer) {
  ...
  while (i < text.length) {
    ...
    if (rest.startsWith("\x1b[Z")) {
      events.push({ type: "key", name: "shift-tab" });
      i += 3;
      continue;
    }
...
function initialState(opts) {
  ...
  return {
    ...
    // all left nothing showing where you were.
    missingCursor: 0,
...
const CURSOR_FIELD = { installed: "cursor", notInstalled: "missingCursor" };

function cursorOf(state, section = state.section) {
  return state[CURSOR_FIELD[section]] || 0;
}
...
// Both sections move one entry at a time. The not-installed list is a reference
// list — nothing there is selectable — but it still needs an indicator to show
// where you are while browsing it.
function move(state, delta) {
  const list = items(state);
  if (list.length === 0) return state;
  const last = list.length - 1;
  const field = CURSOR_FIELD[state.section];
  const next = Math.max(0, Math.min(last, cursorOf(state) + delta));
  return clampScroll({ ...state, [field]: next });
}
...
function applyKey(state, key) {
  ...
  if (key === "down" || key === "j")
    return { state: move(state, 1), action: null };
  if (key === "pageup")
    return { state: move(state, -viewportSize(state)), action: null };
  if (key === "pagedown")
    return { state: move(state, viewportSize(state)), action: null };
  ...
  if (key === "?") {
    ...
    if (target === "notInstalled" && state.notInstalled.length === 0)
      return { state, action: null };
    return { state: clampScroll({ ...state, section: target }), action: null };
  }

  if ((key === "u" || key === "U") && state.updateNotice) {
  ...
  if (key === "enter") {
    ...
    return {
      state: { ...state, chosen: chosen.kind },
...
function applyHover(state, row, col) {
  ...
  if (row === 0 && typeof col === "number") {
    const hit = headerTabs(state, Math.max(24, state.width), total).labels.find(
      (l) => col >= l.start && col < l.end,
    );
  ...
  const hover =
    index === undefined ? null : { kind: "row", section: state.section, index };
  return sameHover(state.hover, hover) ? state : { ...state, hover };
}
...
function applyClick(state, row, col) {
  ...
  if (row === 0 && typeof col === "number") {
    ...
    if (hit && hit.key !== state.section) {
      return {
        state: clampScroll({ ...state, section: hit.key }),
        action: null,
      };
    }
    return { state, action: null };
...
function columnWidths(list, width) {
  const longest = (pick, cap) =>
    Math.min(cap, Math.max(4, ...list.map((a) => pick(a).length)));
  let nameWidth = list.length ? longest((a) => a.name, NAME_CAP) : 4;
  const kindWidth = list.length ? longest((a) => a.kind, KIND_CAP) : 4;
  const tagRoom = list.some((a) => a.isSource)
    ? COL_GAP + SOURCE_TAG.length
    : 0;
...
// Padded to the full pane width, never trimmed: a row's background has to reach
// the edge or it reads as a coloured band that stops mid-line.
function agentRow(agent, { marker, width, nameWidth, kindWidth, showTag }) {
  ...
  const row =
    `  ${marker} ` +
    pad(agent.name, nameWidth) +
    gap +
    pad(agent.kind, kindWidth) +
    tag;
  return pad(truncate(row, width), width);
}
...
function headerTabs(state, width, total) {
  const tabs = [
    { key: "installed", short: "installed", count: state.installed.length },
    {
      key: "notInstalled",
      short: "not installed",
      count: state.notInstalled.length,
    },
  ...
  const build = (withCounts, withCounter) => {
    ...
    const bodies = tabs.map(
      (t) => `${t.short}${withCounts ? ` (${t.count})` : ""}`,
    );
  ...
  return (
    [build(true, true), build(true, false), build(false, false)].find(
      (v) => v.length <= width,
    ) || build(false, false)
  );
}
...
function headerLine(state, width, styled, total) {
  ...
  const text =
    ...
    variant.labels
      .map((l) => {
        const hovered =
          state.hover &&
          state.hover.kind === "tab" &&
          state.hover.key === l.key;
        if (l.active)
          return style(
            l.text,
            hovered ? t.activeTabHover : t.activeTab,
            styled,
          );
  ...
  if (!variant.withCounter) {
    return variant.plain.length <= width
      ? text
      : truncate(variant.plain, width);
  }
...
function footerLine(state, width, styled) {
  ...
  if (state.updateNotice && state.updateNotice.available) {
    ...
    return style(truncate(notice, width), t.accentText, styled);
  }
  ...
  const chips =
    state.section === "installed"
      ? [
          { label: " ⏎ hand off ", primary: true },
          { label: " esc cancel ", primary: false },
        ]
      : [
          { label: " tab back ", primary: true },
          { label: " esc cancel ", primary: false },
        ];
  const short = [
    { label: " ⏎ ", primary: true },
...
function renderFrame(state, opts = {}) {
  ...
  const summary = truncate(
    `${state.contextLine}${arrow}${state.destination}`,
    width - 2,
  );
  ...
  for (const [offset, agent] of visible.entries()) {
    ...
    // Every row is already padded to the pane width, so every background — the
    // selection, the hover, and the ordinary rows in between — reaches the edge.
    if (isCursor) {
