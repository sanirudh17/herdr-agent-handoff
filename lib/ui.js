...
function headerTabs(state, width, total) {
  ...
  const build = (withCounts, withCounter) => {
    ...
    const labels = tabs.map((t) => {
      const text = ` ${t.short}${withCounts ? ` (${t.count})` : ""} `;
      const label = { key: t.key, text, active: t.key === state.section, start: column, end: column + text.length };
...
function headerLine(state, width, styled, total) {
  ...
  const text = " " + variant.labels
    .map((l) => style(l.text, l.active ? t.activeTab : t.inactiveTab, styled))
    .join(" ") + " ";
