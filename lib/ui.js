...
function initialState(opts) {
  const {
    ...
    theme = null, updateNotice = opts.updateNotice || null,
  } = opts;
  return {
    contextLine,
    destination,
...
function footerLine(state, width, styled) {
  if (state.updateNotice && state.updateNotice.available) {
    ...
    if (notice.length + dismissChip.length + 1 <= width) {
      ...
      return style(notice, t.accentText, styled) + " ".repeat(gap) + style(dismissChip, t.primaryChip, styled);
    }
    ...
    if (shortNotice.length + dismissChip.length + 1 <= width) {
      ...
      return style(shortNotice, t.accentText, styled) + " ".repeat(gap) + style(dismissChip, t.primaryChip, styled);
    }
