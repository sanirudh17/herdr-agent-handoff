...
test("the picker pane command does not rely on the process cwd", () => {
  const block = MANIFEST.split(/^\[\[panes\]\]/m).find((b) =>
    /id\s*=\s*"picker"/.test(b),
  );
...
test("the manifest ships no diagnostic probe pane", () => {
  assert.ok(
    !/id\s*=\s*"probe"/.test(MANIFEST),
    "the debug probe pane must not be committed",
  );
});
...
test("declared action and pane ids are unique", () => {
  ...
  assert.equal(
    new Set(ids).size,
    ids.length,
    `duplicate ids in manifest: ${ids.join(", ")}`,
  );
});
