...
function setup() {
  ...
  fs.writeFileSync(requestPath, JSON.stringify({
    ...
    notInstalled: Array.from({ length: 18 }, (_, i) => ({ kind: `u${i}`, name: `Unavailable ${i}` })),
  }));
...
test("the rendered frame shows the roster, counter and place labels", () => {
  ...
  assert.ok(!res.stdout.includes("Handoff to Agent"), "the popup border already carries the title");
...
test("tab reaches the not-installed roster inside the real picker", () => {
  ...
  assert.equal(res.status, 0, res.stderr);
  assert.match(res.stdout, /Unavailable 17/, "the last missing agent must be reachable");
});
