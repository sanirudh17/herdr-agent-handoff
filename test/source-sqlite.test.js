...
test("extract never modifies the source database", { skip: SKIP }, () => {
  ...
  assert.equal(
    out.opened,
    "direct",
    "a healthy database should be read in place",
  );
});

test(
  "extract can read from a copy when a direct open is impossible",
  { skip: SKIP },
  () => {
    const dbPath = buildDb();
    const workDir = fs.mkdtempSync(path.join(os.tmpdir(), "handoff-work-"));
...
test(
  "extractToBuffer returns the same bytes extract writes, and touches no disk",
  { skip: SKIP },
  () => {
    const dbPath = buildDb();
...
test(
  "extractToBuffer cleans up after itself when it has to fall back to a copy",
  { skip: SKIP },
  () => {
    const dbPath = buildDb();
    ...
    assert.ok(
      out.body.length > 0,
      "the rows still come back even via the copy path",
    );
  },
);
