  ...
  for (const agent of ["gemini", "kiro", "amp", "maki"]) {
    assert.throws(
      ...
      (err) => {
        ...
        assert.equal(err.reason, `${agent} reports no session identity to Herdr, so it cannot be a handoff source`);
        return true;
      }
    );
  }
...
test("pi: a missing reference recovers to the pane's one session file by cwd", () => {
  const home = tmpHome();
  const file = writeFile(path.join(piSessionDir(home), `2026-07-24T00-00-00-000Z_${ID}.jsonl`), BODY);
  const got = resolve({ agent: "pi", sessionRef: null, cwd: CWD, homedir: home, env: {} });
  assert.equal(got.path, file);
  assert.equal(got.lines, 2);
});
...
test("pi: two sessions for one cwd refuses to guess which is active", () => {
  const home = tmpHome();
  writeFile(path.join(piSessionDir(home), `2026-07-24T00-00-00-000Z_a.jsonl`), BODY);
  writeFile(path.join(piSessionDir(home), `2026-07-24T00-00-00-000Z_b.jsonl`), BODY);
  assert.throws(
    () => resolve({ agent: "pi", sessionRef: null, cwd: CWD, homedir: home, env: {} }),
    (err) => err instanceof SourceContextUnavailable && /refusing to guess/.test(err.reason)
  );
});
...
test("pi: no session recorded for the cwd still reports the missing reference", () => {
  ...
  assert.throws(
    () => resolve({ agent: "pi", sessionRef: null, cwd: CWD, homedir: home, env: {} }),
    (err) => err instanceof SourceContextUnavailable && /reported no session reference/.test(err.reason)
  );
});

test("claude: a missing reference recovers to the pane's one transcript by cwd", () => {
  const home = tmpHome();
  const file = writeFile(path.join(claudeProjectDir(home), `${ID}.jsonl`), BODY);
  const got = resolve({ agent: "claude", sessionRef: null, cwd: CWD, homedir: home, env: {} });
  assert.equal(got.path, file);
});
...
test("claude: two transcripts for one project refuse to guess", () => {
  ...
  assert.throws(
    () => resolve({ agent: "claude", sessionRef: null, cwd: CWD, homedir: home, env: {} }),
    (err) => err instanceof SourceContextUnavailable && /refusing to guess/.test(err.reason)
  );
});
...
test("codex: recovery scans rollouts for the first-line payload cwd and takes the newest", () => {
  ...
  const oldFile = writeFile(path.join(dir, "rollout-2026-07-10T10-00-00-a.jsonl"), ROLLOUT("a"));
  const newFile = writeFile(path.join(dir, "rollout-2026-07-10T16-00-00-b.jsonl"), ROLLOUT("b"));
  setMtime(oldFile, 1000);
  setMtime(newFile, 2000);
  const got = resolve({ agent: "codex", sessionRef: null, cwd: CWD, homedir: home, env: {} });
...
test("codex: rollouts of equal age refuse to guess", () => {
  const home = tmpHome();
  const dir = path.join(home, ".codex", "sessions", "2026", "07", "10");
  const a = writeFile(path.join(dir, "rollout-2026-07-10T10-00-00-a.jsonl"), ROLLOUT("a"));
  const b = writeFile(path.join(dir, "rollout-2026-07-10T16-00-00-b.jsonl"), ROLLOUT("b"));
  setMtime(a, 1000);
  setMtime(b, 1000);
  assert.throws(
    () => resolve({ agent: "codex", sessionRef: null, cwd: CWD, homedir: home, env: {} }),
    (err) => err instanceof SourceContextUnavailable && /equally recent/.test(err.reason)
  );
});
...
test("cline: sessions.db matches the pane cwd and prefers the open session", () => {
  ...
  db.prepare("INSERT INTO sessions VALUES (?,?,?,?,?)")
    .run("old-closed", "2026-07-24T09:00:00.000Z", "2026-07-24T10:00:00.000Z", CWD, CWD);
  db.prepare("INSERT INTO sessions VALUES (?,?,?,?,?)")
    .run("new-open", "2026-07-25T09:00:00.000Z", null, CWD, CWD);
  db.close();
  const file = writeFile(path.join(data, "sessions", "new-open", "new-open.messages.json"), BODY);
  const got = resolve({ agent: "cline", sessionRef: null, cwd: CWD, homedir: home, env: {} });
  assert.equal(got.path, file);
});
...
test("cline: without an open row the newest closed session is used", () => {
  ...
  db.exec("CREATE TABLE sessions (session_id TEXT, started_at TEXT, ended_at TEXT, cwd TEXT, workspace_root TEXT)");
  db.prepare("INSERT INTO sessions VALUES (?,?,?,?,?)")
    .run("old", "2026-07-24T09:00:00.000Z", "2026-07-24T10:00:00.000Z", CWD, CWD);
  db.prepare("INSERT INTO sessions VALUES (?,?,?,?,?)")
    .run("new", "2026-07-25T09:00:00.000Z", "2026-07-25T10:00:00.000Z", CWD, CWD);
  db.close();
  const file = writeFile(path.join(data, "sessions", "new", "new.messages.json"), BODY);
  const got = resolve({ agent: "cline", sessionRef: null, cwd: CWD, homedir: home, env: {} });
  assert.equal(got.path, file);
});
...
test("cline: metadata files are the fallback when the database is absent", () => {
  ...
  writeFile(
    ...
    JSON.stringify({ session_id: "meta-session", workspace_root: CWD, started_at: "2026-07-25T09:00:00.000Z" })
  );
  const file = writeFile(path.join(data, "sessions", "meta-session", "meta-session.messages.json"), BODY);
  const got = resolve({ agent: "cline", sessionRef: null, cwd: CWD, homedir: home, env: {} });
  assert.equal(got.path, file);
});
...
test("grok: the search index resolves a cwd case-insensitively into a content snapshot", () => {
  ...
  const { DatabaseSync } = require("node:sqlite");
  const db = new DatabaseSync(dbPath);
  db.exec("CREATE TABLE session_docs (session_id TEXT, cwd TEXT, updated_at INTEGER, content TEXT)");
  db.prepare("INSERT INTO session_docs VALUES (?,?,?,?)")
    .run("old", CWD.toLowerCase(), 1000, "old body");
  db.prepare("INSERT INTO session_docs VALUES (?,?,?,?)")
    .run("fresh", CWD, 2000, "fresh body");
  db.close();
  const got = resolve({ agent: "grok", sessionRef: null, cwd: CWD, homedir: home, env: {} });
...
test("grok: sessions of equal age for one cwd refuse to guess", () => {
  ...
  const db = new DatabaseSync(dbPath);
  db.exec("CREATE TABLE session_docs (session_id TEXT, cwd TEXT, updated_at INTEGER, content TEXT)");
  db.prepare("INSERT INTO session_docs VALUES (?,?,?,?)").run("a", CWD, 1000, "a");
  db.prepare("INSERT INTO session_docs VALUES (?,?,?,?)").run("b", CWD, 1000, "b");
  db.close();
  assert.throws(
    () => resolve({ agent: "grok", sessionRef: null, cwd: CWD, homedir: home, env: {} }),
    (err) => err instanceof SourceContextUnavailable && /equally recent/.test(err.reason)
  );
});
...
test("cline and grok name why no record exists instead of claiming no identity", () => {
  const home = tmpHome();
  assert.throws(
    () => resolve({ agent: "cline", sessionRef: null, cwd: CWD, homedir: home, env: {} }),
    (err) => err instanceof SourceContextUnavailable && /no persisted session/.test(err.reason)
  );
  assert.throws(
    () => resolve({ agent: "grok", sessionRef: null, cwd: CWD, homedir: home, env: {} }),
    (err) => err instanceof SourceContextUnavailable && /no indexed session/.test(err.reason)
  );
});
...
test("an ambiguity in the primary search is never papered over by recovery", () => {
  ...
  assert.throws(
    () => resolve({ agent: "claude", sessionRef: { kind: "id", value: ID }, cwd: CWD, homedir: home, env: {} }),
    (err) => err instanceof SourceContextUnavailable && /more than one/.test(err.reason)
  );
});
...
test("opencode: without a reference, recovery keys on the pane cwd", () => {
  const home = tmpHome();
  const dbPath = writeFile(path.join(home, ".local", "share", "opencode", "opencode.db"), "");
  const { DatabaseSync } = require("node:sqlite");
  const db = new DatabaseSync(dbPath);
  db.exec("CREATE TABLE session (id TEXT, directory TEXT, time_updated INTEGER, parent_id TEXT, time_archived INTEGER)");
  db.prepare("INSERT INTO session VALUES (?,?,?,?,?)").run("ses_old", "C:/Users/sanir/Herdr Plugin", 1000, null, null);
  db.prepare("INSERT INTO session VALUES (?,?,?,?,?)").run("ses_fwd", "C:/Users/sanir/Herdr plugin", 2000, null, null);
  db.prepare("INSERT INTO session VALUES (?,?,?,?,?)").run("ses_sub", "C:/Users/sanir/Herdr Plugin", 3000, "ses_old", null);
  db.prepare("INSERT INTO session VALUES (?,?,?,?,?)").run("ses_arch", "C:/Users/sanir/Herdr Plugin", 4000, null, 5000);
  db.prepare("INSERT INTO session VALUES (?,?,?,?,?)").run("ses_other", "C:/Other", 9999, null, null);
  db.close();
  const got = resolve({ agent: "opencode", sessionRef: null, cwd: CWD, homedir: home, env: {} });
...
test("opencode: sessions of equal activity for one cwd refuse to guess", () => {
  ...
  const db = new DatabaseSync(dbPath);
  db.exec("CREATE TABLE session (id TEXT, directory TEXT, time_updated INTEGER, parent_id TEXT, time_archived INTEGER)");
  db.prepare("INSERT INTO session VALUES (?,?,?,?,?)").run("ses_a", "C:/Users/sanir/Herdr Plugin", 7, null, null);
  db.prepare("INSERT INTO session VALUES (?,?,?,?,?)").run("ses_b", "C:/Users/sanir/Herdr Plugin", 7, null, null);
  db.close();
  assert.throws(
    () => resolve({ agent: "opencode", sessionRef: null, cwd: CWD, homedir: home, env: {} }),
    (err) => err instanceof SourceContextUnavailable && /equally recent/.test(err.reason)
  );
});
...
test("opencode: no session for the pane cwd reports the missing store record", () => {
  ...
  db.exec("CREATE TABLE session (id TEXT, directory TEXT, time_updated INTEGER, parent_id TEXT, time_archived INTEGER)");
  db.prepare("INSERT INTO session VALUES (?,?,?,?,?)").run("ses_x", "C:/Elsewhere", 1, null, null);
  db.close();
  assert.throws(
    () => resolve({ agent: "opencode", sessionRef: null, cwd: CWD, homedir: home, env: {} }),
    (err) => err instanceof SourceContextUnavailable && /no session reference/.test(err.reason)
  );
});
...
test("a source kind whose store is absent says the store is missing, not the identity", () => {
  ...
  try {
    assert.throws(
      () => resolve({ agent: "hermes", sessionRef: { kind: "id", value: ID }, homedir: home, env: {} }),
      (err) => err instanceof SourceContextUnavailable &&
