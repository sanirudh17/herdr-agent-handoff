...
test("upstream aliases resolve to the same palette", () => {
  assert.deepEqual(
    theme.paletteFor("catppuccin-mocha"),
    theme.paletteFor("catppuccin"),
  );
  assert.deepEqual(
    theme.paletteFor("tokyonight"),
    theme.paletteFor("tokyo-night"),
  );
  assert.deepEqual(
    theme.paletteFor("solarized-dark"),
    theme.paletteFor("solarized"),
  );
  assert.deepEqual(
    theme.paletteFor("dawn"),
    theme.paletteFor("rose-pine-dawn"),
  );
  assert.deepEqual(
    theme.paletteFor("Tokyo Night"),
    theme.paletteFor("tokyo-night"),
...
test("a missing or unreadable config falls back to the default theme", () => {
  ...
  assert.equal(
    theme.resolveTheme("/no/such/config.toml").name,
    theme.DEFAULT_THEME,
  );
});
...
test("styles emit truecolour SGR built from the resolved palette", () => {
  const s = theme.styles(
    theme.resolveTheme(configFile('[theme]\nname = "catppuccin"\n')),
  );
  assert.match(
    s.activeTab,
    /\x1b\[48;2;137;180;250m/,
    "active tab sits on the accent",
  );
  assert.match(
    s.activeTab,
    /\x1b\[38;2;24;24;37m/,
    "with the panel background as text",
  );
  ...
  assert.match(
    s.hoverRow,
    /\x1b\[48;2;49;50;68m/,
    "the pointer gets the quieter surface",
  );
